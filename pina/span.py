...
class Span(Location):
    def __init__(self, span_dict):
        ...
        for k, v in span_dict.items():
            if isinstance(v, (int, float)):
                ...
                self.fullvector_[k] = v
            else:
                raise TypeError

    @property
    def variables(self):
        return list(self.fixed_.keys()) + list(self.range_.keys()) + list(self.fullvector_.keys())

    def update(self, new_span):
        self.fixed_.update(new_span.fixed_)
        self.range_.update(new_span.range_)
        self.fullvector_.update(new_span.fullvector_)
    ...
    def sample_tensor_given_pts(self, n, variables='all'):
        
        if variables == 'all':
            variables = list(self.fullvector_.keys()) + list(self.fixed_.keys())
        result = None
        for variable in variables:
            if variable in self.fullvector_.keys():
                pts_variable = self.fullvector_[variable]
            ...
            if result is None:
                ...
                result = torch.cat((result, pts_variable), dim=1)
        result = LabelTensor(result, variables)
        return result
