...
for i in range(len(list)):
    mat = scipy.io.loadmat('MassSpring_Dis_{}_OnyData_{}.mat'.format(method,list[i]))
    actual = Benchmark_data[:,2]
    pred = mat['predicted_output_{}_OnyData_{}'.format(method,list[i])][:,0]
...
plt.legend(fontsize=20) # 9 means top center
plt.tight_layout()
plt.savefig('Plots_comparison/L2norm_withoutPINN_h.pdf', dpi=300)